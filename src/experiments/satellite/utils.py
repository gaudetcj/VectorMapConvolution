...
def get_x_max_y_min(im_id: str) -> Tuple[float, float]:
    ...
    if _x_max_y_min is None:
        with open('./data/satellite_data/grid_sizes.csv') as f:
...
def get_wkt_data() -> Dict[str, Dict[int, str]]:
    ...
    if _wkt_data is None:
        _wkt_data = {}
        with open('./data/satellite_data/train_wkt_v4.csv') as f:
...
def load_image(im_id: str, rgb_only=False, align=True) -> np.ndarray:
    im_rgb = tiff.imread('./data/satellite_data/three_band/{}.tif'.format(im_id)).transpose([1, 2, 0])
    if rgb_only:
        return im_rgb
    im_p = np.expand_dims(tiff.imread('./data/satellite_data/sixteen_band/{}_P.tif'.format(im_id)), 2)
    im_m = tiff.imread('./data/satellite_data/sixteen_band/{}_M.tif'.format(im_id)).transpose([1, 2, 0])
    im_a = tiff.imread('./data/satellite_data/sixteen_band/{}_A.tif'.format(im_id)).transpose([1, 2, 0])
    w, h = im_rgb.shape[:2]
    if align:
...
def _get_alignment(im_ref, im_to_align, key):
    if key is not None:
        cached_path = Path('./data/satellite_data/align_cache').joinpath('{}.alignment'.format(key))
    ...
    cc, warp_matrix = cv2.findTransformECC(
        im_ref, im_to_align, warp_matrix, warp_mode, criteria, inputMask=None, gaussFiltSize=1)
    if key is not None:
