...
from models.vectormapconv.vectormap_layers import VectorMapConv, VectorMapTransposeConv, VectorMapBatchNorm2d


@attr.s(slots=True)
class HyperParams:
    classes = attr.ib(default=list(range(10)))
    net = attr.ib(default='VectorUNet')
    ...
    augment_rotations = attr.ib(default=20.0)  # degrees
...
def vectorconv3x3(dvm, in_, out):
    return VectorMapConv(dvm, in_, out, 3, padding=1)
...
class VectorUNetModule(nn.Module):
    def __init__(self, hps: HyperParams, in_: int, out: int):
        super().__init__()
        self.conv1 = vectorconv3x3(hps.n_channels, in_, out)
        self.conv2 = vectorconv3x3(hps.n_channels, out, out)
        self.bn = hps.bn
        self.activation = getattr(F, hps.activation)
        if self.bn:
            self.bn1 = nn.BatchNorm2d(out)
...
class VectorUNet(BaseNet):
    module = VectorUNetModule
