...
import experiments.satellite.utils as utils
from experiments.satellite.sat_models import HyperParams
import experiments.satellite.sat_models as models
...
class Model:
    ...
    def train_step(self, x, y, dist_y):
        ...
        cls_losses = [float(l.data) for l in losses]
        loss = losses[0]
        for l in losses[1:]:
            loss += l
    ...
    def load_image(self, im_id: str) -> Image:
        ...
        im_cache = Path('./data/satellite_data/im_cache')
    ...
    def _train_on_feeds(self, gen_batch, n_batches: int, no_mp: bool):
        ...
        def log():
            ...
            with open('./data/satellite_data/train_log.txt', 'a+') as f:
                f.seek(0)
                ...
                f.write(str(self._format_jaccard(jaccard_stats)))
    ...
    def validate_on_images(self, valid_images: List[Image],
        ...
        with open('./data/satellite_data/val_log.txt', 'a+') as f:
            f.seek(0)
            # If file is not empty then append '\n'
            data = f.read(100)
            if len(data) > 0:
...
def main():
    ...
    mask_stats = json.loads(Path('./data/satellite_data/cls-stats.json').read_text())
